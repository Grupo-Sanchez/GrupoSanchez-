...
const ModificarEliminarProveedor = () => {
  ...
  const fetchData = async () => {
    await axios.get('http://Localhost:178.128.67.247/api/marcas').then((response) => {
  ...
  const modifyMarca = async (values) => {
    try {
      ...
      setModal(false);
      await axios
        .put(`http://Localhost:178.128.67.247/api/marcas/${modificar._id}`, payload)
        .then((response) => {
  ...
  const onDelete = async (i) => {
    ...
    if (isDeletable) {
      await axios.delete(`http://Localhost:178.128.67.247/api/marcas/${data[i]._id}`);
