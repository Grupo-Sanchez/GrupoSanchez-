...
const Departamento = require('../models/departmentModel');

router.post('/create', upload.single('imagenDepartamento'), (req, res, next) => {
  console.log('=====>', req.file);
  Departamento.find({ nombre: req.body.nombre })
    .exec()
    .then((marca) => {
      if (marca.length >= 1) {
        ...
        const marca = new Departamento({
          _id: new mongoose.Types.ObjectId(),
          nombre: req.body.nombre,
          descripcion: req.body.descripcion,
          imagenDepartamento: `http://localhost:3001/${req.file.path}`,
        });
        marca
          .save()
          .then((result) => {
...
router.get('/', (req, res, next) => {
  Departamento.find()
    .select('nombre descripcion imagenDepartamento')
    .exec()
    .then((docs) => {
      const response = {
        count: docs.length,
        departamentos: docs.map((doc) => ({
          ...
          imagenDepartamento: doc.imagenDepartamento,
          _id: doc._id,
          request: {
            type: 'GET',
            url: `http://localhost:3001/api/departamentos/${doc._id}`,
          },
        })),
      };
...
router.get('/:departamentoId', (req, res, next) => {
    ...
    .then((doc) => {
      ...
      if (doc) {
        res.status(200).json({
          ...
          request: {
            type: 'GET',
            url: 'http://localhost:3001/api/departamentos',
          },
        });
      } else {
...
router.delete('/:departamentoId', (req, res, next) => {
  const id = req.params.departamentoId;
  Departamento.remove({ _id: id })
    .exec()
    .then((result) => {
      res.status(200).json({
